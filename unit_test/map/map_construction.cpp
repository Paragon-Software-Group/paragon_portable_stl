...
// *
// * Project="Paragon Portable STL" File="map_construction.cpp"
// * 
...
TEST(map, constructor_default) {
  ...
  {
    ::portable_stl::map<std::int32_t, double> mp;
...
TEST(map, constructor_default_noexcept) {
  static_cast<void>(test_info_);

  {
    using t_container = ::portable_stl::map<test_common_helper::MoveOnly, test_common_helper::MoveOnly>;
  ...
  }
  {
    ...
    using t_container = ::portable_stl::map<test_common_helper::MoveOnly,
  ...
  }
  {
    using t_container = ::portable_stl::map<test_common_helper::MoveOnly,
                                            test_common_helper::MoveOnly,
                                            test_map_helper::some_comp<test_common_helper::MoveOnly>>;
...
class map_rec {
public:
  ::portable_stl::map<std::int32_t, map_rec>                 m;
  ::portable_stl::map<std::int32_t, map_rec>::iterator       i;
  ::portable_stl::map<std::int32_t, map_rec>::const_iterator ci;

  // These reverse_iterator specializations require map_rec to be complete
  // ::portable_stl::map<std::int32_t, map_rec>::reverse_iterator       ri;
  // ::portable_stl::map<std::int32_t, map_rec>::const_reverse_iterator cri;
};
} // namespace test_map_helper
...
TEST(map, constructor_compare) {
  static_cast<void>(test_info_);

  ::portable_stl::map<std::int32_t, double, t_compare> const mp(t_compare(3));
...
TEST(map, constructor_allocator) {
  static_cast<void>(test_info_);

  ::portable_stl::map<std::int32_t, double, t_compare, t_allocator> const mp(t_allocator(4));
...
TEST(map, constructor_compare_allocator) {
  static_cast<void>(test_info_);

  ::portable_stl::map<std::int32_t, double, t_compare, t_allocator> const mp(t_compare(3), t_allocator(4));
...
TEST(map, constructor_iter_iter) {
  static_cast<void>(test_info_);

  {
    ...
    ASSERT_EQ(3, mp.size());
    ASSERT_EQ(3, ::portable_stl::distance(mp.begin(), mp.end()));
    EXPECT_EQ(t_map_value_type(1, 1), *mp.begin());
    EXPECT_EQ(t_map_value_type(2, 1), *::portable_stl::next(mp.begin()));
    EXPECT_EQ(t_map_value_type(3, 1), *::portable_stl::next(mp.begin(), 2));
  }
  ...
  // bad alloc
  {
    ...
    try {
      ::portable_stl::map<std::int32_t const, double, ::portable_stl::less<std::int32_t>, t_allocator> mp(
        test_map_helper::arr, test_map_helper::arr_end);
      ASSERT_TRUE(false);
...
TEST(map, make_map_iter_iter) {
  static_cast<void>(test_info_);

  {
    auto result = ::portable_stl::map<std::int32_t, double>::make_map(test_map_helper::arr, test_map_helper::arr_end);
    ASSERT_TRUE(result);
  ...
  // bad alloc
  {
    ...
    auto result
      = ::portable_stl::map<std::int32_t const, double, ::portable_stl::less<std::int32_t>, t_allocator>::make_map(
        test_map_helper::arr, test_map_helper::arr_end);
...
TEST(map, constructor_iter_iter_comp) {
  static_cast<void>(test_info_);

  {
    ::portable_stl::map<std::int32_t, double, t_compare> mp(
      test_map_helper::arr, test_map_helper::arr_end, t_compare(3));
  ...
  // bad alloc
  {
    ...
    try {
      ::portable_stl::map<std::int32_t const, double, t_compare, t_allocator> mp(
        test_map_helper::arr, test_map_helper::arr_end, t_compare(3));
      ASSERT_TRUE(false);
    } catch (::portable_stl::bad_alloc<> &) {
    }
...
TEST(map, make_map_iter_iter_comp) {
  static_cast<void>(test_info_);

  {
    auto result = ::portable_stl::map<std::int32_t, double, t_compare>::make_map(
      test_map_helper::arr, test_map_helper::arr_end, t_compare(3));
    ASSERT_TRUE(result);
  ...
  // bad alloc
  {
    ...
    auto result = ::portable_stl::map<std::int32_t const, double, t_compare, t_allocator>::make_map(
      test_map_helper::arr, test_map_helper::arr_end, t_compare(3));
...
TEST(map, constructor_iter_iter_allocator) {
  static_cast<void>(test_info_);

  {
    ::portable_stl::map<std::int32_t, double, t_compare, t_allocator> mp(
      test_map_helper::arr, test_map_helper::arr_end, t_allocator(4));
  ...
  // bad alloc
  {
    ...
    try {
      ::portable_stl::map<std::int32_t const, double, t_compare, t_allocator2> mp(
        test_map_helper::arr, test_map_helper::arr_end, alloc);
      ASSERT_TRUE(false);
    } catch (::portable_stl::bad_alloc<> &) {
    }
  }
...
TEST(map, make_map_iter_iter_allocator) {
  static_cast<void>(test_info_);

  {
    auto result = ::portable_stl::map<std::int32_t, double, t_compare, t_allocator>::make_map(
      test_map_helper::arr, test_map_helper::arr_end, t_allocator(4));
    ASSERT_TRUE(result);
  ...
  // bad alloc
  {
    ...
    auto result = ::portable_stl::map<std::int32_t const, double, t_compare, t_allocator2>::make_map(
      test_map_helper::arr, test_map_helper::arr_end, alloc);
...
TEST(map, constructor_iter_iter_comp_alloc) {
  static_cast<void>(test_info_);

  {
    ::portable_stl::map<std::int32_t, double, t_compare, t_allocator> mp(
      test_map_helper::arr, test_map_helper::arr_end, t_compare(3), t_allocator(4));
  ...
  // bad alloc
  {
    ...
    try {
      ::portable_stl::map<std::int32_t const, double, t_compare, t_allocator2> mp(
        test_map_helper::arr, test_map_helper::arr_end, t_compare(3), alloc);
      ASSERT_TRUE(false);
    } catch (::portable_stl::bad_alloc<> &) {
    }
  }
...
TEST(map, make_map_iter_iter_comp_alloc) {
  static_cast<void>(test_info_);
  ...
  // bad alloc
  {
    ...
    auto result = ::portable_stl::map<std::int32_t, double, t_compare, t_allocator2>::make_map(
      test_map_helper::arr, test_map_helper::arr_end, t_compare(3), alloc);
...
TEST(map, constructor_copy) {
  ...
  {
    ::portable_stl::map<std::int32_t, double, t_compare, t_allocator> mp0(
      test_map_helper::arr, test_map_helper::arr_end, t_compare(3), t_allocator(4));

    ::portable_stl::map<std::int32_t, double, t_compare, t_allocator> mp = mp0;
    ...
    ASSERT_EQ(3, mp0.size());
    ASSERT_EQ(t_compare(3), mp0.key_comp());
    ASSERT_EQ(t_allocator(4), mp0.get_allocator());
    ASSERT_EQ(3, ::portable_stl::distance(mp0.begin(), mp0.end()));
    EXPECT_EQ(t_map_value_type(1, 1), *mp0.begin());
    EXPECT_EQ(t_map_value_type(2, 1), *::portable_stl::next(mp0.begin()));
    EXPECT_EQ(t_map_value_type(3, 1), *::portable_stl::next(mp0.begin(), 2));
  }
  ...
  // bad allocation
  {
    ...
    ::portable_stl::map<std::int32_t, double, t_compare, t_allocator2> mp0(
      test_map_helper::arr, test_map_helper::arr_end, t_compare(3), alloc);

    try {
      ::portable_stl::map<std::int32_t, double, t_compare, t_allocator2> mp = mp0;
      ASSERT_TRUE(false);
    } catch (::portable_stl::bad_alloc<> &) {
    }
  }
...
TEST(map, constructor_copy_alloc) {
  static_cast<void>(test_info_);

  {
    ...
    ::portable_stl::map<std::int32_t, double, t_compare, t_allocator> mp(mp0, t_allocator(5));
  ...
  {
    ::portable_stl::map<std::int32_t, double, t_compare, t_allocator2> mp0(
      test_map_helper::arr, test_map_helper::arr_end, t_compare(3), t_allocator2(4));

    test_allocator_helper::test_allocator_statistics stat;
    stat.m_throw_after = 1;
    t_allocator2 alloc(&stat);
    try {
      ::portable_stl::map<std::int32_t, double, t_compare, t_allocator2> mp(mp0, alloc);
      ASSERT_TRUE(false);
    } catch (::portable_stl::bad_alloc<> &) {
    }
  }
...
TEST(map, constructor_move) {
...

  {
    ...
    ::portable_stl::map<std::int32_t, double, t_compare, t_allocator2> mp = std::move(mp0);
...
TEST(map, constructor_move_alloc) {
  static_cast<void>(test_info_);

  {
     ...
     * @brief Map alias.
     */
    using t_map = ::portable_stl::map<t_move_only, t_move_only, t_compate2, t_allocator3>;
...

  {
     ...
     * @brief Map alias.
     */
    using t_map = ::portable_stl::map<std::int32_t, t_value, t_compate2, t_allocator3>;
...

    {
      ...
      ASSERT_LE(test_common_helper::Counter_base::gConstructed, (std::int32_t)(num + 6 + mp1.size()));

      {
        t_map mp4(std::move(mp2), t_allocator3(5));
        ASSERT_GE(test_common_helper::Counter_base::gConstructed, (std::int32_t)(num + 6));
        ASSERT_LE(test_common_helper::Counter_base::gConstructed, (std::int32_t)(num + 6 + mp1.size() + mp2.size()));
        ASSERT_EQ(mp4, mp3);
        ASSERT_TRUE(mp2.empty());
      }
      ASSERT_GE(test_common_helper::Counter_base::gConstructed, (std::int32_t)(num + 3));
      ASSERT_LE(test_common_helper::Counter_base::gConstructed, (std::int32_t)(num + 3 + mp1.size() + mp2.size()));
    }
...
TEST(map, constructor_move_noexcept) {
  static_cast<void>(test_info_);
  ...
  {
    using t_container = ::portable_stl::map<t_move_only, t_move_only>;
  ...
  }
  {
    ...
    using t_container = ::portable_stl::
      map<t_move_only, t_move_only, std::less<t_move_only>, ::portable_stl::allocator<t_map_value_type2>>;
    ASSERT_TRUE(std::is_nothrow_move_constructible<t_container>{});
  }

  // todo(plotitsyn)
  // {
  //   using t_container = ::portable_stl::map<t_move_only, t_move_only, test_map_helper::some_comp<t_move_only>>;

  //   ASSERT_FALSE(std::is_nothrow_move_constructible<t_container>{});
  // }
}

TEST(map, constructor_init_list) {
  static_cast<void>(test_info_);
  ...
  // bad alloc
  {
    ...
    try {
      ::portable_stl::map<std::int32_t, double, ::portable_stl::less<std::int32_t>, t_allocator> mp({
        {1,   1},
        {1, 1.5},
        {1,   2},
        {2,   1},
...
TEST(map, constructor_init_list_comp) {
  static_cast<void>(test_info_);
...
TEST(map, constructor_init_list_alloc) {
  static_cast<void>(test_info_);
...
TEST(map, constructor_init_list_comp_alloc) {
  static_cast<void>(test_info_);
...
TEST(map, copy_assign) {
...

  {
    ...
    ASSERT_EQ(*mp.begin(), t_map_value_type(1, 1));
    ASSERT_EQ(*::portable_stl::next(mp.begin()), t_map_value_type(2, 1));
    ASSERT_EQ(*::portable_stl::next(mp.begin(), 2), t_map_value_type(3, 1));
    ...
    ASSERT_EQ(*mp0.begin(), t_map_value_type(1, 1));
    ASSERT_EQ(*::portable_stl::next(mp0.begin()), t_map_value_type(2, 1));
    ASSERT_EQ(*::portable_stl::next(mp0.begin(), 2), t_map_value_type(3, 1));
  }
  {
    t_map_value_type ar[] = {t_map_value_type(1, 1), t_map_value_type(2, 1), t_map_value_type(3, 1)};

    ::portable_stl::map<int, double> mp(ar, ar + sizeof(ar) / sizeof(ar[0]));

    ::portable_stl::map<int, double> *ptr = &mp;
  ...
  ASSERT_TRUE(test_map_helper::balanced_allocs());
  {
    t_map_value_type ar[] = {t_map_value_type(1, 1),
                             t_map_value_type(1, 1.5),
                             t_map_value_type(1, 2),
    ...
    using t_allocatorT = test_map_helper::counting_allocatorT<t_map_value_type>;

    ::portable_stl::map<std::int32_t, double, t_compare, t_allocatorT> mp0(
      ar, ar + sizeof(ar) / sizeof(ar[0]), t_compare(5), t_allocatorT(1));

    ::portable_stl::map<std::int32_t, double, t_compare, t_allocatorT> mp(
      ar, ar + sizeof(ar) / sizeof(ar[0]) / 2, t_compare(3), t_allocatorT(2));

    mp = mp0;
    ASSERT_EQ(mp.key_comp(), t_compare(5));
    ASSERT_EQ(mp.size(), 3);
    ASSERT_EQ(::portable_stl::distance(mp.begin(), mp.end()), 3);
    ASSERT_EQ(*mp.begin(), t_map_value_type(1, 1));
    ASSERT_EQ(*::portable_stl::next(mp.begin()), t_map_value_type(2, 1));
    ASSERT_EQ(*::portable_stl::next(mp.begin(), 2), t_map_value_type(3, 1));

    ASSERT_EQ(mp0.key_comp(), t_compare(5));
    ASSERT_EQ(mp0.size(), 3);
    ASSERT_EQ(::portable_stl::distance(mp0.begin(), mp0.end()), 3);
    ASSERT_EQ(*mp0.begin(), t_map_value_type(1, 1));
    ASSERT_EQ(*::portable_stl::next(mp0.begin()), t_map_value_type(2, 1));
    ASSERT_EQ(*::portable_stl::next(mp0.begin(), 2), t_map_value_type(3, 1));
  }
  ...
  ASSERT_TRUE(test_map_helper::balanced_allocs());
  {
    ...
    using t_allocatorF = test_map_helper::counting_allocatorF<t_map_value_type>;

    ::portable_stl::map<std::int32_t, double, t_compare, t_allocatorF> mp0(
      ar, ar + sizeof(ar) / sizeof(ar[0]), t_compare(5), t_allocatorF(100));

    ::portable_stl::map<std::int32_t, double, t_compare, t_allocatorF> mp(
...
TEST(map, move_assign) {
  ...
  using t_map_move_only         = ::portable_stl::map<t_move_only, t_move_only, t_compare2, t_allocator3>;
...
TEST(map, assign_initializer_list) {
  static_cast<void>(test_info_);
  ...
  }
  {
    ...
    using t_allocator3 = test_allocator_helper::test_allocator<::portable_stl::tuple<std::int32_t const, std::int32_t>>;
    using t_map = ::portable_stl::map<std::int32_t, std::int32_t, ::portable_stl::less<std::int32_t>, t_allocator3>;
    {
      ...
      ASSERT_EQ(1, mp.size());
      ASSERT_EQ(4, ::portable_stl::get<0>(*mp.begin()));
    }
    ASSERT_EQ(0, alloc_stats.m_alloc_count);
  }
}
