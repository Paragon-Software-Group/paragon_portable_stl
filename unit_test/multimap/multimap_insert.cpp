...
TEST(multimap, insert_value_type) {
...

  {
    ...
    ASSERT_EQ(1, mp.size());
    ASSERT_EQ(2, ::portable_stl::get<0>(*iter));
    ...
    ASSERT_EQ(mp.begin(), iter);
    ASSERT_EQ(2, mp.size());
    ASSERT_EQ(1, ::portable_stl::get<0>(*iter));
    ASSERT_EQ(1.5, ::portable_stl::get<1>(*iter));
    ...
    ASSERT_EQ(::portable_stl::prev(mp.end()), iter);
    ASSERT_EQ(3, mp.size());
    ASSERT_EQ(3, ::portable_stl::get<0>(*iter));
    ASSERT_EQ(3.5, ::portable_stl::get<1>(*iter));

    // == 4
    t_value_type const value4(3, 4.5);
    result = mp.insert(value4);
    ASSERT_TRUE(result);
    ...
    ASSERT_EQ(4, mp.size());
    ASSERT_EQ(3, ::portable_stl::get<0>(*iter));
    ASSERT_EQ(4.5, ::portable_stl::get<1>(*iter));
  }
...
template<class t_map, class t_value_like_type> void map_insert_rv_test() {
  ...
  ASSERT_EQ(1, mp.size());
  ASSERT_EQ(2, ::portable_stl::get<0>(*iter));
  ASSERT_EQ(5, ::portable_stl::get<1>(*iter));
  ...
  ASSERT_EQ(2, mp.size());
  ASSERT_EQ(1, ::portable_stl::get<0>(*iter));
  ASSERT_EQ(3, ::portable_stl::get<1>(*iter));
  ...
  ASSERT_EQ(3, mp.size());
  ASSERT_EQ(3, ::portable_stl::get<0>(*iter));
  ASSERT_EQ(4, ::portable_stl::get<1>(*iter));
  ...
  ASSERT_EQ(4, mp.size());
  ASSERT_EQ(3, ::portable_stl::get<0>(*iter));
  ASSERT_EQ(7, ::portable_stl::get<1>(*iter));
}
} // namespace test_multimap_helper
...
TEST(multimap, insert_rvalue_type) {
  static_cast<void>(test_info_);
  ...
  // value_type_like&&
  test_multimap_helper::map_insert_rv_test<::portable_stl::multimap<std::int32_t, t_move_only>,
  ...
  // implicit value_type&&
  {
    ::portable_stl::multimap<std::int32_t, t_move_only> mp;
    ...
    ASSERT_EQ(1, mp.size());
    ASSERT_EQ(2, ::portable_stl::get<0>(*iter));
    ASSERT_EQ(2, ::portable_stl::get<1>(*iter));
    ...
    ASSERT_EQ(2, mp.size());
    ASSERT_EQ(1, ::portable_stl::get<0>(*iter));
    ASSERT_EQ(1, ::portable_stl::get<1>(*iter));
    ...
    // == 4
    result = mp.insert({3, t_move_only(7)});
    ASSERT_TRUE(result);
...
TEST(multimap, insert_pos_value_type) {
  static_cast<void>(test_info_);
...
template<class t_map, class t_value_like_type> void map_insert_pos_rv_test() {
  ...
  // == 4
  result = mp.insert(mp.end(), t_value_like_type(3, 4.5));
  ASSERT_TRUE(result);
...
TEST(multimap, insert_pos_rvalue_type) {
  static_cast<void>(test_info_);
  ...
  // value_type_like&&
  test_multimap_helper::map_insert_pos_rv_test<::portable_stl::multimap<std::int32_t, t_move_only>,
                                               ::portable_stl::tuple<std::int32_t, t_move_only>>();
  ...
  // implicit value_type&&
  {
    ...
    // == 4
    result = mp.insert(mp.end(), {3, t_move_only(4)});
    ASSERT_TRUE(result);
...
TEST(multimap, insert_iter_iter) {
  static_cast<void>(test_info_);
...
TEST(multimap, insert_initializer_list) {
  static_cast<void>(test_info_);
  ...
  ASSERT_EQ(::portable_stl::get<1>(*::portable_stl::next(mp.begin(), 3)), 4.5);
}
...
typename t_map::node_type node_factory(typename t_map::key_type const &key, typename t_map::mapped_type const &mapped) {
  ...
  // get created node
  return mp.extract(iter);
}
} // namespace test_multimap_helper

// insert_return_type insert(node_type&&);
TEST(multimap, insert_node_type) {
  static_cast<void>(test_info_);

  /**
   * @brief Map alias.
   */
  using t_map2 = ::portable_stl::multimap<std::int32_t, std::int32_t>;

  t_map2 mp;

  auto *nf = &test_multimap_helper::node_factory<t_map2>;

  for (std::int32_t i{0}; i != 10; ++i) {
    typename t_map2::node_type node = nf(i, i + 1);
    ASSERT_FALSE(node.empty());

    typename t_map2::iterator iter = mp.insert(std::move(node));

    ASSERT_TRUE(node.empty());
    ASSERT_EQ(mp.find(i), iter);
    ASSERT_NE(mp.end(), iter);
  }
  ...
  { // Insert empty node.
    ...
    auto                       iter = mp.insert(std::move(def));
  ...
  { // Insert duplicate node.
    ...
    auto                       iter  = mp.insert(std::move(dupl));
    ASSERT_TRUE(dupl.empty());
    ASSERT_NE(mp.end(), iter);
    ASSERT_EQ(42, ::portable_stl::get<1>(*iter));
  }

  ASSERT_EQ(11, mp.size());
  ASSERT_EQ(2, mp.count(0));

  for (std::int32_t i{1}; i != 10; ++i) {
    ASSERT_EQ(1, mp.count(i));
    ASSERT_EQ(i + 1, ::portable_stl::get<1>(*mp.find(i)));
  }
}

// insert_return_type insert(node_type&&);
TEST(multimap, insert_pos_node_type) {
  static_cast<void>(test_info_);
  ...
  for (std::int32_t i{0}; i != 10; ++i) {
    ...
    auto iter = mp.insert(mp.end(), std::move(node));

    ASSERT_TRUE(node.empty());
    ASSERT_EQ(prev + 1, mp.size());
    ASSERT_EQ(mp.find(i), iter);
    ASSERT_EQ(::portable_stl::get<0>(*iter), i);
    ASSERT_EQ(::portable_stl::get<1>(*iter), i + 1);
  }
...
TEST(multimap, emplace) {
  ...
  {
    ::portable_stl::multimap<std::int32_t, t_default_only> mp;
    ...
    ASSERT_EQ(0, ::portable_stl::get<0>(*iter));
    ASSERT_EQ(t_default_only(), ::portable_stl::get<1>(*iter));
    ...
    ASSERT_EQ(::portable_stl::next(mp.begin()), iter);
    ASSERT_EQ(2, mp.size());
    ...
    ASSERT_EQ(3, t_default_only::count);
  }
  ASSERT_EQ(0, t_default_only::count);
  ...
  // bad allocation
  {
    ...
    ::portable_stl::multimap<std::int32_t, t_default_only, t_compare, t_allocator> mp(alloc);
    mp.emplace(
      ::portable_stl::piecewise_construct_t{}, ::portable_stl::forward_as_tuple(5),
      ::portable_stl::forward_as_tuple());
    mp.emplace(
      ::portable_stl::piecewise_construct_t{}, ::portable_stl::forward_as_tuple(6),
      ::portable_stl::forward_as_tuple());
  ...
  {
    ::portable_stl::multimap<std::int32_t, test_common_helper::Emplaceable> mp;

    // == 1
    auto result = mp.emplace(
      ::portable_stl::piecewise_construct_t{}, ::portable_stl::forward_as_tuple(2),
      ::portable_stl::forward_as_tuple());
    ASSERT_TRUE(result);
    ...
    ASSERT_EQ(2, ::portable_stl::get<0>(*iter));
    ASSERT_EQ(test_common_helper::Emplaceable(), ::portable_stl::get<1>(*iter));
    ...
    // iter = result.value();
    // ASSERT_TRUE(::portable_stl::get<1>(iter));

    // insert_iter = ::portable_stl::get<0>(iter);
    ...
    // iter = result.value();
    // ASSERT_FALSE(::portable_stl::get<1>(iter)); // false
